...
public class FailureSimulator implements Runnable {
	...
	public FailureSimulator(int x_max, int y_max, HashMap<Integer, SLA> slas, HashMap<Integer, VM> vms, Timer timer,
			EdgeController controller, CommonType handType, CommonType simuType) {
		super();
		this.x_max = x_max;
		this.y_max = y_max;
		this.slas = slas;
				...
				while (t == thisThread) {
					...
					try {
						...
						if(getSimuType() == CommonType.PM){
	            			...
	            			System.out.println("No such failure simulation type. Retry...");
	            		}
	...
	public boolean recurMigration(boolean isFailed, int id, int count, CommonType simuType) { 
	    if (count > 1 || !isFailed) return isFailed; 
